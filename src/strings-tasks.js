...
function sumOfCodes(str) {
  return str ? str.split('').reduce((acc, v) => acc + v.charCodeAt(), 0) : 0;
}
...
function isPalindrome(str) {
  const palidrom = str
    ...
    .join('');
  return (
    str.toLowerCase().replace(/[ ,!?]/g, '') === palidrom.toLocaleLowerCase()
  );
}
...
function encodeToRot13(str) {
  ...
  const newStr = str.replace(reg, (el) => letters[letters.indexOf(el) + 13]);
  return newStr;
}
